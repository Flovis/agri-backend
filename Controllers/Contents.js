...
const addContents = async (req, res, next) => {
    const { title, language, cycle, productName, tag, category, description } =
        req.body;
    let categoryID;
    let languageID;
    let cycleID;
    ...
    const file = `${req.protocol}://${req?.get("host")}/${req?.file?.path}`;
    ...
    if (
        ...
        if (categoryID && languageID && cycleID && productID && tagID) {
            try {
                ...
                if (createContent) {
                    try {
                        const contentCategory = await db.Content_Tags.create({
                            TagId: tagID,
